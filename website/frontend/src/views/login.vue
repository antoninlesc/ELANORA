      ...
      <div class="login-card">
        <div class="login-header">
          ...
          <div class="instance-info">
            <span class="instance-badge">{{ userStore.user?.instance_name || 'Default' }} Instance</span>
            <p class="instance-desc">
              {{ $t('login.connect_to_access', { instanceName: userStore.user?.instance_name || 'Default' }) }}
            </p>
          </div>
        </div>
        ...
        <form class="login-form" @submit.prevent="handleLogin">
          ...
          <button type="submit" class="btn-primary login-btn" :disabled="userStore.authState.loading">
            <span v-if="userStore.authState.loading">{{ $t('login.logging_in') }}</span>
        ...
        <div class="institution-login">
          <button class="btn-secondary login-btn" @click="handleSSOLogin" :disabled="userStore.authState.loading">
            🏛️ <span>{{ $t('login.login_with_sso', { provider: ssoProvider }) }}</span>
          </button>
        </div>
        
        <div class="login-footer">
          <p>{{ $t('login.no_account') }} <a href="#" @click="requestAccess">{{ $t('login.request_access') }}</a></p>
          <p class="invitation-text">
            {{ $t('login.have_invitation') }} 
            <router-link to="/invitation">{{ $t('login.join_project') }}</router-link>
          </p>
        </div>
      </div>
