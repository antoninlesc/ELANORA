  ...
  try {
    ...
    if (response.data && response.data.length > 0) {
      ...
      const validResults = response.data.filter((result) => {
        ...
        return (
          resultCityName &&
          resultCityName.toLowerCase() === cityName.toLowerCase()
        );
      });
...
export const validatePostalCode = async (postalCode, countryCode) => {
  if (!postalCode || !countryCode) {
    return {
      success: false,
      error: 'Postal code and country code are required',
    };
  }
      ...
      data: {
        ...
        message:
          postalCode.trim().length > 0
            ? 'Format not validated for this country'
  ...
  return {
    ...
    data: {
      ...
      message: isValid
        ...
        : 'Invalid postal code format for this country',
    },
  };
};
    ...
    if (response.data && response.data.length > 0) {
      ...
      const validResults = response.data.filter((result) => {
        ...
        const resultCity =
          ...
          address.municipality;
        return (
          resultCity && resultCity.toLowerCase() === cityName.toLowerCase()
        );
      });
 ...
 * @returns {Promise<Object>} Response with validation result
 */
export const validateStreetNumberInStreet = async (
  streetNumber,
  streetName,
  cityName,
  countryCode
  ...
  try {
    ...
    if (response.data && response.data.length > 0) {
      ...
      const validResults = response.data.filter((result) => {
        ...
        const cityMatches =
          resultCity && resultCity.toLowerCase() === cityName.toLowerCase();
        const streetMatches = resultStreet
          ?.toLowerCase()
      ...
      if (validResults.length > 0) {
        ...
        const streetExistsResults = response.data.filter((result) => {
          ...
          const streetMatches =
            resultStreet &&
            resultStreet.toLowerCase().includes(streetName.toLowerCase());
