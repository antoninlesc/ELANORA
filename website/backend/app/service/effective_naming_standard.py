...
async def assign_effective_standard(
    db: AsyncSession, project_id: int, project_file_type_id: int, naming_standard_id: int, location_id: int
):
    try:
        result = await set_effective_standard(db, project_id, project_file_type_id, naming_standard_id, location_id)
        await db.commit()
        return result
...
async def unassign_effective_standard(
    ...
    try:
        await remove_effective_standard(db, project_id, project_file_type_id, location_id)
        await db.commit()
    except Exception as e:
        await db.rollback()
        raise e
