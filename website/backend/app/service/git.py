...
class GitService:
    ...
    async def _process_single_bulk_rename(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not get filename for elan_id {elan_id}")
    ...
    async def download_files(
        self, project_name: str, elan_ids: list[int], db: AsyncSession
    ) -> StreamingResponse:
        """Download selected files as a ZIP archive.

        Args:
            project_name: Name of the project.
            elan_ids: List of elan_ids to download.
            db: Database session.
        ...
        # Fetch files by elan_ids, ensuring they belong to the project
        files = await get_elan_files_by_ids_and_project(
            db, elan_ids, project.project_id
        )
        if len(files) != len(elan_ids):
            raise ValueError("Some files not found or do not belong to the project.")

        # Create ZIP in memory
        zip_buffer = BytesIO()
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            for elan_file in files:
                abs_path = elan_file.absolute_file_path
                if not Path(abs_path).exists():
                    logger.warning(
                        f"File '{elan_file.filename}' not found on disk at {abs_path}, skipping from download."
                    )
                    continue
                # Add file to ZIP with its name (preserve structure if needed)
                zip_file.write(abs_path, elan_file.filename)
