...
async def link_component_to_accepted_value(db: AsyncSession, component_template_id: int, accepted_value_id: int):
    filters = {
...
async def unlink_component_from_accepted_value(db: AsyncSession, component_template_id: int, accepted_value_id: int):
    return await DatabaseUtils.delete_by_filter(
        db, ComponentAcceptedValue,
        component_template_id=component_template_id,
        accepted_value_id=accepted_value_id
    )
...
async def delete_for_orphaned_templates(db: AsyncSession):
    try:
        logger.info("Starting orphaned ComponentAcceptedValue cleanup for orphaned templates...")
        referenced_ids_result = await db.execute(select(StandardComponent.component_template_id))
        referenced_ids = {row[0] for row in referenced_ids_result}
        orphaned_templates_result = await db.execute(
            select(ComponentTemplate.id).where(~ComponentTemplate.id.in_(referenced_ids))
        )
        ...
        if orphaned_template_ids:
            ...
            logger.info(f"Deleted {deleted} ComponentAcceptedValue rows for orphaned templates.")
            await db.flush()
        else:
            logger.info("No orphaned templates found for ComponentAcceptedValue cleanup.")
